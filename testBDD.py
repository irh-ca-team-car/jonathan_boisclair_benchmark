...
model_1 = Segmenter.named("unet+mit_b0").adaptTo(dataset).to(device)
def mdl():
    return model_1
name = "unet+mit_b0"
model_ctr = mdl
...
try:
    model.load_state_dict(torch.load("car/lanes.pth", map_location=device), strict=False)
except:
    pass
for i in range(100):
    if i>=5:
    ...
    for b in Batch.of(dataset, 8):
        ...
        torchvision.io.write_png((m.onImage(b[0])*255.0).byte(),"progress.png")

        optim.step()
        optim.zero_grad()
        torch.save(model.state_dict(), "car/lanes.pth")
